...
type clientAdapter struct {
	connection.RemoteK8sControllerClient
}

func newClientAdapter(c client.Client, clusterID string) *clientAdapter {
	return &clientAdapter{
		RemoteK8sControllerClient: connection.RemoteK8sControllerClient{
			ClusterID: clusterID,
			Client:    c,
		},
	}
...
func (suite *RGControllerTestSuite) TestPVCRemapping() {
	suite.Run("SuccessfulRemapping", suite.testSuccessfulPVCRemapping)
	suite.Run("PVCNotFound", suite.testPVCNotFound)
	suite.Run("PVNotFound", suite.testPVNotFound)
	suite.Run("DifferentRetentionPolicies", suite.testDifferentRetentionPolicies)
	suite.Run("SingleClusterScenario", suite.testSingleClusterScenario)
	suite.Run("MultiClusterScenario", suite.testMultiClusterScenario)
}

func (suite *RGControllerTestSuite) testSuccessfulPVCRemapping() {
	ctx := context.Background()

	rg := suite.getRGWithSyncComplete(suite.driver.RGName)
	suite.createSCAndRG(suite.getTypicalSC(), rg)

	localPVC := &v1.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "local-pvc",
			Namespace: "default",
			Annotations: map[string]string{
				replicationPrefix + "remotePV":               "remote-pv",
				replicationPrefix + "remoteStorageClassName": "remote-sc",
			},
		},
		Spec: v1.PersistentVolumeClaimSpec{
			VolumeName:       "local-pv",
			StorageClassName: pointer.String("local-sc"),
		},
	}
	suite.NoError(suite.client.Create(ctx, localPVC))
	...
	suite.NoError(suite.client.Create(ctx, localPV))
	...
	suite.NoError(suite.client.Create(ctx, remotePV))
	adapter := newClientAdapter(suite.client, "test-cluster-id")
	err := swapPVC(ctx, adapter, "local-pvc", "default", "remote-pv", suite.driver.RGName, suite.reconciler.Log)
	suite.NoError(err)

	updatedPVC := &v1.PersistentVolumeClaim{}
	suite.NoError(suite.client.Get(ctx, types.NamespacedName{Name: "local-pvc", Namespace: "default"}, updatedPVC))
	suite.Equal("remote-pv", updatedPVC.Spec.VolumeName)
	suite.Equal("remote-sc", *updatedPVC.Spec.StorageClassName)

	updatedLocalPV := &v1.PersistentVolume{}
	suite.NoError(suite.client.Get(ctx, types.NamespacedName{Name: "local-pv"}, updatedLocalPV))
	suite.Nil(updatedLocalPV.Spec.ClaimRef)

	updatedRemotePV := &v1.PersistentVolume{}
	suite.NoError(suite.client.Get(ctx, types.NamespacedName{Name: "remote-pv"}, updatedRemotePV))
	suite.Equal("local-pvc", updatedRemotePV.Spec.ClaimRef.Name)
	suite.Equal("default", updatedRemotePV.Spec.ClaimRef.Namespace)
}

func (suite *RGControllerTestSuite) testPVCNotFound() {
	ctx := context.Background()
	adapter := newClientAdapter(suite.client, "test-cluster-id")
	err := swapPVC(ctx, adapter, "non-existent-pvc", "default", "remote-pv", suite.driver.RGName, suite.reconciler.Log)
	suite.Error(err)
	suite.Contains(err.Error(), "not found")
}

func (suite *RGControllerTestSuite) testPVNotFound() {
	ctx := context.Background()
	localPVC := &v1.PersistentVolumeClaim{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "local-pvc",
			Namespace: "default",
			Annotations: map[string]string{
				replicationPrefix + "remotePV": "non-existent-pv",
			},
		},
		Spec: v1.PersistentVolumeClaimSpec{
			VolumeName: "local-pv",
	...
	suite.NoError(suite.client.Create(ctx, localPVC))
	adapter := newClientAdapter(suite.client, "test-cluster-id")
	err := swapPVC(ctx, adapter, "local-pvc", "default", "non-existent-pv", suite.driver.RGName, suite.reconciler.Log)
	suite.Error(err)
	suite.Contains(err.Error(), "not found")
}

func (suite *RGControllerTestSuite) testDifferentRetentionPolicies() {
	ctx := context.Background()
	...
	updatedLocalPV := &v1.PersistentVolume{}
	suite.NoError(suite.client.Get(ctx, types.NamespacedName{Name: "local-pv"}, updatedLocalPV))
	suite.Equal(v1.PersistentVolumeReclaimRetain, updatedLocalPV.Spec.PersistentVolumeReclaimPolicy)

	updatedRemotePV := &v1.PersistentVolume{}
	suite.NoError(suite.client.Get(ctx, types.NamespacedName{Name: "remote-pv"}, updatedRemotePV))
	suite.Equal(v1.PersistentVolumeReclaimRetain, updatedRemotePV.Spec.PersistentVolumeReclaimPolicy)
}

func (suite *RGControllerTestSuite) testSingleClusterScenario() {
	ctx := context.Background()
...
func (suite *RGControllerTestSuite) testMultiClusterScenario() {
	ctx := context.Background()
	...
	remoteClient, err := suite.reconciler.Config.GetConnection(suite.driver.RemoteClusterID)
	suite.NoError(err)

	remotePV := &v1.PersistentVolume{
		ObjectMeta: metav1.ObjectMeta{Name: "remote-pv"},
	}
	suite.NoError(remoteClient.CreatePersistentVolume(ctx, remotePV))
	adapter := newClientAdapter(suite.client, "test-cluster-id")
	err = swapPVC(ctx, adapter, "local-pvc", "default", "remote-pv", suite.driver.RGName, suite.reconciler.Log)
	suite.NoError(err)
	...
	updatedRemotePV := &v1.PersistentVolume{}
	remotePV, err = remoteClient.GetPersistentVolume(ctx, "remote-pv")
	suite.NoError(err)
	suite.Equal("local-pvc", remotePV.Spec.ClaimRef.Name)
	suite.Equal("default", remotePV.Spec.ClaimRef.Namespace)
	suite.Equal("local-pvc", updatedRemotePV.Spec.ClaimRef.Name)
	suite.Equal("default", updatedRemotePV.Spec.ClaimRef.Namespace)
}
